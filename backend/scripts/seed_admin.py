...
async def seed_admin(email: str, password: str) -> None:
    database_url = settings.DATABASE_URL or "postgresql+asyncpg://dental_dev:dental_dev_pass@localhost:5432/dental_clinic_dev"
    ...
    try:
        async with session_factory() as session:
            ...
            print(f"Admin account seeded successfully: {email}")
    except Exception as e:
        print(f"Error seeding admin account: {e}", file=sys.stderr)
        sys.exit(1)
    finally:
